-- GUIHUB.lua
-- This script creates two GUIs:
-- 1. The persistent toggle button (always visible, draggable, and cannot be deleted by the player)
-- 2. The main GUIHUB which contains the title, menu, linkBox, copy & close buttons.
-- The Close button in the main GUIHUB now only hides it, and the toggle button can re-open it.

local placeId = game.PlaceId
local player = game.Players.LocalPlayer

-------------------------------------------
-- Create the persistent ScreenGui for toggle button
-------------------------------------------
local persistentGui = Instance.new("ScreenGui")
persistentGui.Name = "PersistentToggleGUI"
persistentGui.ResetOnSpawn = false
persistentGui.Parent = player:WaitForChild("PlayerGui")

-------------------------------------------
-- Create the main ScreenGui for the GUIHUB
-------------------------------------------
...
-------------------------------------------
-- Create Main GUIHUB Frame (draggable)
-------------------------------------------
...
-------------------------------------------
-- "Work: 100%" Label (Below Welcome Label)
-------------------------------------------
local workLabel = Instance.new("TextLabel")
workLabel.Size = UDim2.new(0, 100, 0, 20)
workLabel.Position = UDim2.new(1, -110, 0, 45)
workLabel.BackgroundTransparency = 1
workLabel.Text = "Work: 100%"
workLabel.Font = Enum.Font.Gotham
workLabel.TextSize = 13
workLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
workLabel.Parent = mainFrame
...
local function loadPlaceConfig()
    if placeHubMapping[placeId] then
        ...
        if success then
            loadstring(response)()
            print("95% - Loaded external script from: " .. fileUrl)
        else
...
-- Close button functionality (now only hides mainFrame)
closeButton.MouseButton1Click:Connect(function()
    mainFrame.Visible = false
end)
print("98% - Copy and Close functions set.")

-------------------------------------------
-- Toggle Button (Bottom Left) to show/hide the GUIHUB
-------------------------------------------
local toggleButton = Instance.new("ImageButton")
toggleButton.Size = UDim2.new(0, 40, 0, 40)
toggleButton.Position = UDim2.new(0, 5, 1, -45)
toggleButton.Image = "https://i.postimg.cc/nhFRcCDF/IMG-4431.png" -- use the same image
...
if success then
    loadstring(response)()
    print("External GUI script loaded successfully.")
else
    warn("Failed to load external GUI script from: " .. rawLink)
end

print("GUI loaded successfully at 100%!")
